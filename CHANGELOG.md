...
### Added
- New PrestaShop Webservice fetch in the upload tab as an alternative data source to uploading a CSV: `POST /api/fetch/prestashop` downloads products straight from the store by EAN and/or reference (batch `OR` filters), with optional `description`/`images` presence filters, importing the first 50 matching products (one row per combination, with combination-level price/wholesale price/stock and product-level name/descriptions/brand/category/tax). Fetched data becomes the working dataset, replacing any uploaded CSVs (a confirmation dialog warns about the replacement, and uploading a CSV afterwards discards the fetched data); `DELETE /api/fetch/prestashop` removes it and re-locks the downstream tabs. The dashboard treats the fetched dataset like a CSV upload, so validation, image matching, AI suggestions and review run against it.
