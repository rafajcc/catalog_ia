...
- `ean` and `reference` are now required CSV columns: rows without a valid EAN (8 or 13 digits) or without a reference are rejected by the parser, and the normalizer flags them as missing required fields too. A reference longer than 64 characters (PrestaShop limit) is also rejected.
