...
const CSV_CONTENT = [
  'ean,name,reference,price,quantity',
  '8412345678901,Producto A,REF-A,19.99,10',
  '8412345678902,Producto B,REF-B,9.50,5',
  'bad-ean,Producto C,REF-C,3.00,1'
].join('\n');
...
describe('API routes', () => {
  ...
  it('generates AI text suggestions for missing fields', async () => {
    ...
    const suggested = await request(app)
      ...
      .send({ provider: 'mock', language: 'es', enabled_fields: ['name', 'description'] });
